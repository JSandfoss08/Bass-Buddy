...
//
//	Dervied from https://gist.github.com/darktable/2317063
...
public static class SavWav {
	...
	static void WriteHeader(FileStream fileStream, List<AudioClip> clips) {
		...
		int clipsCnt = clips.Count;
		
		int totalChannels = 0;
		int totalSamples = 0; 
		for(int i = 0; i < clipsCnt; i++){
			totalChannels += clips[i].channels;
			totalSamples += clips[i].samples;
		}

		var hz = clips[0].frequency;
		var channels = totalChannels;
